...
final class SignInWithAppleHelper: NSObject {
    ...
    private var completionHandler: ((SignInWithAppleResult?, Error?) -> Void)? = nil
    ...
    func startSignInWithAppleFlow(viewController: UIViewController? = nil) -> AsyncThrowingStream<SignInWithAppleResult, Error> {
        AsyncThrowingStream { continuation in
            startSignInWithAppleFlow { signInWithAppleResult, error in
                if let error = error {
                    continuation.finish(throwing: error)
                } else if let signInWithAppleResult = signInWithAppleResult {
                    continuation.yield(signInWithAppleResult)
                    continuation.finish()
                } else {
                    continuation.finish(throwing: SignInWithAppleError.badResponse)
                }
            }
        }
    }
    ...
    @MainActor
    private func startSignInWithAppleFlow(viewController: UIViewController? = nil, completion: @escaping (SignInWithAppleResult?, Error?) -> Void) {
        guard let topVC = viewController ?? UIApplication.topViewController() else {
            completion(nil, SignInWithAppleError.noViewController)
            return
        }
